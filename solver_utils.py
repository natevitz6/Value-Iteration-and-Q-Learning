...
# -*- coding: utf-8 -*-
# Partnership? YES
  # Submitting partner: Nathan Vitzthum
  # Other partner: Grace Liu
...
def value_iteration(
    ...
    for s in mdp.nonterminal_states:
        for a in mdp.actions:
            q_table[(s, a)] = sum(mdp.transition(s, a, s_next) * 
                                  (mdp.reward(s, a, s_next) + mdp.config.gamma * v_table[s_next])
                                  for s_next in mdp.all_states)
        
        best_q = max(q_table[(s, a)] for a in mdp.actions)
        max_delta = max(max_delta, abs(v_table[s] - best_q))
        new_v_table[s] = best_q
...
def q_update(
    ...
    # *** BEGIN OF YOUR CODE ***
    best_next_q = max(q_table[(next_state, a)] for a in mdp.actions) if next_state != mdp.terminal else 0
    q_table[(state, action)] += alpha * (reward + mdp.config.gamma * best_next_q - q_table[(state, action)])
...
def extract_v_table(mdp: tm.TohMdp, q_table: tm.QTable) -> tm.VTable:
    ...
    """
    # *** BEGIN OF YOUR CODE ***
    return {s: max(q_table[(s, a)] for a in mdp.actions) if s != mdp.terminal else 0.0 for s in mdp.all_states}
...
def choose_next_action(
    ...
    """
    # *** BEGIN OF YOUR CODE ***
    best_actions = [a for a in mdp.actions if q_table[(state, a)] == max(q_table[(state, a)] for a in mdp.actions)]
...
def custom_epsilon(n_step: int) -> float:
    ...
    """
    # *** BEGIN OF YOUR CODE ***
    return max(0.000001, 1.0 / (1 + 0.00001 * n_step))
...
def custom_alpha(n_step: int) -> float:
    ...
    """
    # *** BEGIN OF YOUR CODE ***
    return max(0.1, 1.0 / (1 + 0.1 * n_step))